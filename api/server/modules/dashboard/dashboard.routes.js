...
export function adminDashboardRoutes(prisma) {
  ...
  router.get('/', async (req, res) => res.json({ data: await service.listAdmin(listSchema.parse(req.query).dataCollectionId) }))
  router.post('/preview', async (req, res) => res.json({ data: await service.previewConfiguration(baseSchema.parse(req.body)) }))
